...
const Accordion = ({ title, content, width, isOpen, onToggle }: AccordionProps) => {
  ...
  return (
    <div
      className={`rounded-[24px] bg-white border border-slate-100 shadow-xl shadow-slate-200/50 flex flex-col ${
        width ? `w-[${width}%]` : "w-full"
      } h-full overflow-hidden`}
    >
      <button
        onClick={toggleAccordion}
        className="flex items-center justify-between w-full p-4 border-b border-slate-50 shrink-0 bg-white/50"
      >
        <p
      ...
      >
        <div className="flex items-start justify-start px-5 pt-5 pb-10">
