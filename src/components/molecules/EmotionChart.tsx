...
const EmotionChart = ({ averageLevel, chartData }: EmotionChartProps) => {
  ...
  return (
    ...
    >
      <div className={`text-6xl font-bold ${style.textColor} mb-3`}>{averageLevel}</div>
      <div className={`font-medium ${style.messageColor}`}>{style.message}</div>
      <div className={`text-sm font-medium ${style.subMessageColor}`}>{style.subMessage}</div>
              ...
              datasets: [
                {
                  ...
                  backgroundColor: style.chartBgColor,
                  tension: 0.4,
                  fill: true,
                },
              ],
