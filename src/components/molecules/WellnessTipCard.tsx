      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col items-start">
            <span className="text-green-900 font-bold text-lg">오늘의 마음 관리</span>
            <div className="flex items-center gap-1.5 mt-0.5">
              {getCategoryIcon(tip.category)}
              <span className="text-green-600 text-xs font-medium">{tip.category}</span>
            </div>
          </div>
        </div>
      </div>
      <div className="mt-4 text-gray-700 text-base leading-relaxed">{tip.text}</div>
    </motion.div>
  );
};
