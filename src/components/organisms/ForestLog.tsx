...
const ForestLog = () => {
  ...
  const { content: selectedContent, response: selectedResponse } = useSelectTreeStore();
  ...
  const selectedItem = forestData.find(
    (item) => item.content === selectedContent && item.response === selectedResponse
  );
    ...
    <>
      {selectedItem ? (
        <div className="divide-y divide-gray-100 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          <LogForm key={selectedItem.id} data={selectedItem} onDelete={refreshData} />
        </div>
      ) : (
        <motion.div 
        ...
        >
          ...
          >
            <p className="text-base sm:text-lg font-medium text-slate-700 text-center">위의 나무를 선택해주세요</p>
            <p className="text-xs sm:text-sm text-slate-400 text-center mt-1.5 sm:mt-2 leading-relaxed">
              당신의 소중한 생각이 기록된<br />나무를 확인할 수 있어요
            </p>
          </motion.div>
        </motion.div>
      )}
